...
public class Util {
    ...
    public static String getDateStamp(Calendar cal) {
        return cal.get(Calendar.DAY_OF_MONTH) + "/" +
                (cal.get(Calendar.MONTH) + 1) + "/" + // MONTH is zero-based, so add 1
                cal.get(Calendar.YEAR); // Directly use the YEAR as an int
    }
    ...
    public static int[] parseDateStamp(String s) {
        ...
        int[] date = new int[3];
        date[0] = Integer.parseInt(s.substring(0, i1)); // Parse day
        date[1] = Integer.parseInt(s.substring(i1 + 1, i2)); // Parse month
        date[2] = Integer.parseInt(s.substring(i2 + 1)); // Parse year
        return date;
